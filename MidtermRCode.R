...
#Changing to binary
Fuel_type_num <- ifelse(car.df$Fuel_Type=="Diesel",0,1)
head(Fuel_type_num)
...
model <- lm(Price~ Age_08_04 + KM + Fuel_Type, data = train.data) 

tr.res <- data.frame(train.data$Price, model$fitted.values, model$residuals)
head(tr.res)
